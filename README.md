...
| `helm` | `$HELM_REPOSITORY_CONFIG`, or `repositories.yaml` under `$HELM_CONFIG_HOME`, `$XDG_CONFIG_HOME/helm`, `~/.config/helm`, `~/Library/Preferences/helm` or `%APPDATA%\helm` | Helm has a list of named chart repositories rather than one registry, so the backend manages the `stable` entry and adds it when it is missing. Every other repository and field survives; comments are dropped, because the file is round-tripped through a YAML parser. |
| `docker` | `$REGTOOL_DOCKER_DAEMON_JSON`, or `/etc/docker/daemon.json` on Linux and `~/.docker/daemon.json` on macOS and Windows | Rewrites the `registry-mirrors` array; pointing it at `https://registry-1.docker.io` removes the key, which is what talking to Docker Hub directly means. Other keys are preserved, but JSON has no comments so the file comes back with two-space indentation and sorted keys. The system-wide file usually needs root on Linux, and the daemon only picks the change up after a restart. |
